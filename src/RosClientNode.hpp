...
class RosClientNode : public QObject {
  ...
  std::unordered_map<MsgTypeString, std::function<void(const QByteArray&, const TopicString&)>> pub_fns;
   ...
   * @param msg_type type of the message; can be obtained using ros::message_traits::DataType
   * @param from_topic topic on which message was received
   */
  template <typename T>
  void encode_ros_msg(const T& msg, const std::string& msg_type, const std::string& from_topic) {
    std::cerr << "encoding ROS message" << std::endl;

    flatbuffers::FlatBufferBuilder fbb;
    encode<T>(fbb, msg, msg_type, from_topic);
    Q_EMIT ros_message_encoded(
      QByteArray(reinterpret_cast<const char*>(fbb.GetBufferPointer()), fbb.GetSize()));
  }

  template <typename T>
  void publish_ros_msg(const T& msg, const std::string& topic) {
    if (pubs.count(topic) == 0) {
      // gotcha: it's important that we only publish one type T for any given topic!
      // register_msg_type() handles this by checking for duplicate topic registrations.
 ...
 public Q_SLOTS:
  ...
  void decode_net_message(const QByteArray& data) {
    ...
    // extract metadata
    const fb::MsgWithMetadata* msg = flatbuffers::GetRoot<fb::MsgWithMetadata>(data.data());
 ...
 public:
  ...
  void register_msg_type(const std::string& from_topic) {
    if (subs.count(from_topic) > 0) {
      throw std::runtime_error("Trying to register topic that is already registered. Topics must be unique.");
    }
    const std::string& msg_type = ros::message_traits::DataType<T>().value();
    std::cerr << "registering publisher for " << msg_type << " on topic " << from_topic << std::endl;
    ...
    if (pub_fns.count(msg_type) == 0) {
      pub_fns[msg_type] = [&](const QByteArray& data, const std::string& topic){
