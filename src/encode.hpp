...
template <typename T>
static void encode(FBB& fbb, const T& msg, const std::string& msg_type, const std::string& topic);

template<>
void encode(FBB& fbb, const amrl_msgs::RobofleetStatus& msg, const std::string& msg_type, const std::string& topic) {
  auto metadata = fb::CreateMsgMetadataDirect(fbb, msg_type.c_str(), topic.c_str());
...
template<>
void encode(FBB& fbb, const sensor_msgs::NavSatFix& msg, const std::string& msg_type, const std::string& topic) {
  auto metadata = fb::CreateMsgMetadataDirect(fbb, msg_type.c_str(), topic.c_str());
